...
def learnModelMulti():
    ...
    type_prediction = layers.Dense(nClassesType, activation='sigmoid', name='type_h')(type_prediction)
    ...
    ss = lambda y_true, y_pred: cm.w_categorical_crossentropy(y_true, y_pred, weights=artist_weights_matrix)

    custom_model.compile(optimizer='adam',
                  loss={'artist': ss, 'year': 'mae', 'type': 'binary_crossentropy', 'mat': 'binary_crossentropy'},
...
def MLT_learn_test(input_file = 'AWTableTOP20.csv', model = 'RESNET', h_units = 512,  n_epochs = 40, batch_s = 20, img_s = 200, descr = 'No info'):
    ...
    global LOGS_FOLDER, DIR_IMG, AWTable, artist_weights_matrix, artistsWeightTable, encoder_Artist, encoder_Type, encoder_Mat, year_scaler, nClassesArtist, nClassesType, nClassesMat, train_set, valid_set, test_set, NUM_EPOCHS, BATCH_SIZE, IMG_SIZE, MODEL_NAME, NUM_HIDDEN_UNITS, RUN_DESCR
    ...
    # create matrix
    artist_weights_matrix = np.zeros([nClassesArtist, nClassesArtist])
    #artist_weights_matrix = [[]]
    for c_p, c_t in product(range(nClassesArtist), range(nClassesArtist)):
        if(c_p != c_t):
            artist_weights_matrix[c_t, c_p] = class_weights[c_t]
#        else:
#            artist_weights_matrix[c_t, c_p] = 0
    
    #pd.DataFrame(data = artist_weights_matrix).to_csv('weights_matrix.csv')
...
def debugMetrics_loss():    
    ...
    artist_weights_matrix_one = np.ones_like(artist_weights_matrix)
    loss_abs = cm.categorical_crossentropy_abs(y_true,y_pred_s).eval(session=K.get_session())
    loss_artits_w1 = cm.categorical_crossentropy_w_wrap(artist_weights_matrix_one)
    loss_w1 = loss_artits_w1(y_true,y_pred_s).eval(session=K.get_session())    
    np.testing.assert_almost_equal(loss_abs,loss_w1)
    
    loss_artits_w2 = cm.categorical_crossentropy_w_wrap(artist_weights_matrix)
    loss_w2 = loss_artits_w2(y_true,y_pred_s).eval(session=K.get_session())    

    ss = lambda y_true, y_pred: cm.w_categorical_crossentropy(y_true, y_pred, weights=artist_weights_matrix)
    loss_w2b = ss(y_true,y_pred_s).eval(session=K.get_session())    
...
#MLT_learn_test(input_file = 'AWTableSetAL2.csv', model = 'RESNET', h_units = 512,  n_epochs = 1, batch_s = 10, img_s = 224)
