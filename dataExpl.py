...
def anaYear():
    ...
    AWTable = pd.read_csv('../../data/2_meta_files/AWTableTOP100.csv', keep_default_na=False)
    year_pivot = AWTable.pivot_table(values=['Year_Est'], index=['Artist'], aggfunc=['min' , 'max'])
    year_pivot.columns = ['min', 'max']
    year_pivot['delta'] = year_pivot['max'] - year_pivot['min']
    

def anaTxtTypeMat():

    AWTable = pd.read_csv('../../data/2_meta_files/AWTable.csv', keep_default_na=False)
